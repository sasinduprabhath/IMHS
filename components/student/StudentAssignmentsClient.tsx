        ...
        {activeSubmitAssignment && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => !isSubmitting && setActiveSubmitAssignment(null)}
            ...
            <motion.div
              ...
              className="relative z-10 w-full max-w-lg bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-5 my-8 overflow-hidden"
            >
              {/* Top Accent Gradient Bar */}
              <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-[#0E57A4] via-[#F16726] to-[#0E57A4]" />

              {/* Header */}
              <div className="flex items-start justify-between gap-4 pt-1">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono font-bold text-[#0E57A4] bg-[#0E57A4]/10 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                      {activeSubmitAssignment.course.title || "Coursework"}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400">
                      Max: {activeSubmitAssignment.maxMarks} Marks
                    </span>
                  </div>
                ...
                <button
                  type="button"
                  onClick={() => !isSubmitting && setActiveSubmitAssignment(null)}
                  className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-400 hover:text-slate-700 flex items-center justify-center transition-colors shrink-0 cursor-pointer"
              ...
              <div className="bg-gradient-to-r from-blue-50/80 to-slate-50 border border-blue-100 rounded-2xl p-3.5 flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-[#0E57A4]/10 border border-[#0E57A4]/20 flex items-center justify-center shrink-0 text-[#0E57A4]">
                ...
                <div className="text-xs space-y-0.5">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">
                    Submission time and student ID are cryptographically logged for academic grading.
                  </p>
                </div>
              </div>
              ...
              {submitError && (
                <motion.div
                  ...
                  className="bg-rose-50 border border-rose-200/80 text-rose-700 text-xs p-3.5 rounded-2xl flex items-center gap-2.5 font-medium shadow-xs"
                >
              ...
              {submitSuccess && (
                <motion.div
                  initial={{ opacity: 0, y: -4 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-emerald-50 border border-emerald-200/80 text-emerald-800 text-xs p-3.5 rounded-2xl flex items-center gap-2.5 font-bold shadow-xs"
                >
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-slate-700">
                      Upload Completed File <span className="text-rose-500">*</span>
                    </label>
                  ...
                  {!fileUrl ? (
                    // Upload Dropzone
                    <div className="relative border-2 border-dashed border-slate-300 hover:border-[#0E57A4] bg-slate-50/70 hover:bg-blue-50/20 transition-all duration-200 rounded-2xl p-7 text-center group cursor-pointer">
                      <input
                        type="file"
                      ...
                      <div className="space-y-2.5">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#0E57A4]/15 to-[#0E57A4]/5 border border-[#0E57A4]/20 text-[#0E57A4] flex items-center justify-center mx-auto shadow-xs group-hover:scale-110 transition-transform">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-400 mt-0.5">
                            Supported: {activeSubmitAssignment.allowedFileTypes || "PDF, DOCX, ZIP"}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-slate-50 to-blue-50/40 border border-slate-200/90 rounded-2xl p-4 space-y-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-600 flex items-center justify-center shrink-0 font-bold text-xs">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[11px] text-slate-400 font-mono">
                              {fileSize ? formatBytes(fileSize) : "Ready for review"}
                            </p>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="text-xs text-slate-400 hover:text-rose-600 font-medium px-2 py-1 rounded-lg hover:bg-rose-50 transition-colors shrink-0 cursor-pointer"
                          title="Change file"
                        >
                          Change File
                        </button>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-200/60 text-[11px]">
                        <span className="inline-flex items-center gap-1.5 font-bold text-emerald-700 bg-emerald-100/70 border border-emerald-300/50 px-2.5 py-0.5 rounded-full">
                          <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-[#0E57A4] hover:underline font-semibold flex items-center gap-1"
                        >
                          Preview
                ...
                {/* Assignment Identifier (Readonly / Clean Input) */}
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700">
                    Submission Title / Identifier
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. Pharmacology_Worksheet_01.pdf"
                    value={fileName}
                    onChange={(e) => setFileName(e.target.value)}
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 font-medium outline-none focus:border-[#0E57A4] focus:bg-white transition-all shadow-xs"
                    required
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                  <button
                    ...
                    disabled={isSubmitting}
                    className="px-5 py-2.5 text-xs font-bold text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition-all disabled:opacity-50 cursor-pointer"
                  >
                    Cancel
                  </button>
